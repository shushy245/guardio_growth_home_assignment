...
def test_ids_created_at_increasing_milliseconds_sort_by_creation() -> None:
    earlier = generate_unique_id_at("evt", timestamp_ms=1_000)
    middle = generate_unique_id_at("evt", timestamp_ms=2_000)
    later = generate_unique_id_at("evt", timestamp_ms=3_000)

    assert sorted([later, earlier, middle]) == [earlier, middle, later]


def test_a_thousand_ids_in_the_same_millisecond_are_unique() -> None:
    ids = {generate_unique_id_at("sup", timestamp_ms=5_000) for _ in range(1_000)}
