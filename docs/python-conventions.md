...
| Driver namespaces | `given / get / post / patch / delete / then`. `then` replaces `assert` (Python keyword). Methods return `None`, never a response or value. | review |
...
| Real store only in integration tests | `tests/integration/**` (marker `integration`) hits the compose Postgres; each test runs inside a savepoint that is rolled back. Unit tests never open a DB connection. | pytest marker + review |
