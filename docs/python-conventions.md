...
| Given / When / Then, each phase has one home | **Given**: fixture (`conftest.py`) for shared setup, `driver.given.*` for test-specific; **When**: `driver.when.*` in the body (`driver.get/post/patch.*` on a transport driver), never in a fixture; **Then**: `driver.then.*` after the When. Blank line between phases. | review |
...
| Driver namespaces | `given / when / then` on an **entity** driver, whose subject is a behaviour: `when.the_visitor_records(...)` says what happened, where `post.funnel_event(...)` would name the verb and leave the behaviour to the test body. `given / get / post / patch / delete / then` on a **transport** driver, whose subject *is* the HTTP call (`HttpDriver`, and the k-anonymity proxy, which is one GET). `then` replaces `assert` (Python keyword). Methods return `None`, never a response or value. (Decided at the full-codebase audit, T-13: fourteen entity drivers already read this way and the table described the transport one.) | review |
